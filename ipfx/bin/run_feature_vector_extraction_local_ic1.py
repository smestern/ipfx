...
class CollectFeatureVectorParameters(ags.ArgSchema):
    ...
    input = ags.fields.InputDir(
        description=("Input DIR"),
        default=None,
        allow_none=True
    )
    data_source = ags.fields.String(
        description="Source of NWB files (local)",
        default="local"
        )
...
def validate_sweeps(data_set, sweep_numbers, extra_dur=0.2):
    ...
    for swp in check_sweeps.sweeps:
        ...
        if swp_start is None:
            ...
            stim = np.flatnonzero(np.diff(swp.i))
            try:
                middle = swp.t[stim[-2]+1]
            except:
                middle = swp.t[int(dur-start / 2)]
    if start is None:
    ...
    return good_sweep_numbers, start, end, middle


def preprocess_long_square_sweeps(data_set, sweep_numbers, extra_dur=0.2, subthresh_min_amp=-70.):
    if len(sweep_numbers) == 0:
        raise er.FeatureError("No long square sweeps available for feature extraction")

    good_sweep_numbers, lsq_start, lsq_end, middle = validate_sweeps(data_set, sweep_numbers, extra_dur=extra_dur)
...
def data_for_specimen_id(specimen_id, sweep_qc_option, data_source,
    ...
    try:
        ...
        logging.warning(detail)
        {"error": {"type": "sweep_table", "details": traceback.format_exc(limit=None)}}
    ...
    try:
        ...
        result["latency"] = fv.latency_vector(lsq_features["spiking_sweeps"], lsq_start, lsq_end)
        ...
        for feature in spike_feature_list:
            try:
                ...
                result["spiking_" + feature] = np.full(300,np.nan)
            if feature == 'width':
                result["spiking_width"] = result["spiking_width"]
    except Exception as detail:
